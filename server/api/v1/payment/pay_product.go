...
func (pay_productApi *PayProductApi) GetPayProductList(c *gin.Context) {
	...
	list, total, err := pay_productService.GetPayProductInfoList(pageInfo)
	if err != nil {
		...
		for i := 0; i < len(list); i++ {
			productInfo := list[i]
			count, _ := pay_channelService.GetCountByProductId(fmt.Sprintf("%d", productInfo.ID))
			list[i].ChannelCount = &count
		}
